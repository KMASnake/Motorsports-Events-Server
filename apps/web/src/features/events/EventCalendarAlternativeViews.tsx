...
export function EventCalendarAlternativeViews({ view, date, events, selectedId, onSelect, onMove, onCreateAt }: {
  ...
  if (view === 'agenda') return <section className="events-agenda" aria-label="Agenda des événements">
    {visible.length ? visible.map((event) => <button key={event.id} className={selectedId === event.id ? 'selected' : ''} onClick={() => onSelect(event)}>
      <time>{fullDate(event.starts_at)}</time><strong>{event.name}</strong><span className="event-agenda-brand"><img src={assetRegistry.championship(event.championship_slug,event.championship_logo_url).src} alt=""/>{event.championship_name}{assetRegistry.country(event.country_code).src&&<img src={assetRegistry.country(event.country_code).src!} alt={assetRegistry.country(event.country_code).alt}/>}</span>{event.correction_count ? <b>Corrigé · {event.correction_count}</b> : null}
  ...
  return <section className={`events-time-grid ${view}`}>
    {Array.from({length: days}, (_, index) => { const day = new Date(start); day.setDate(start.getDate()+index); return <div key={day.toISOString()} onDragOver={(e)=>e.preventDefault()} onDrop={(e)=>{const found=events.find(x=>x.id===e.dataTransfer.getData('text/event-id')); if(found){const next=new Date(day);next.setHours(new Date(found.starts_at).getHours(),new Date(found.starts_at).getMinutes());onMove(found,next);}}}>
      ...
      {visible.filter((event)=>new Date(event.starts_at).toDateString()===day.toDateString()).map((event)=><button draggable key={event.id} className={selectedId===event.id?'selected':''} onDragStart={(e)=>e.dataTransfer.setData('text/event-id',event.id)} onClick={()=>onSelect(event)}><span className="event-time-brand"><img src={assetRegistry.championship(event.championship_slug,event.championship_logo_url).src} alt=""/>{assetRegistry.country(event.country_code).src&&<img className="event-chip-flag" src={assetRegistry.country(event.country_code).src!} alt={assetRegistry.country(event.country_code).alt}/>}</span><time>{new Date(event.starts_at).toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit'})}</time><strong>{event.name}</strong></button>)}
    </div>})}
  </section>;
}

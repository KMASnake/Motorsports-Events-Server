...
export type AcquisitionWorkClass='finalization'|'current_global'|'recent_catchup'|'deep_history';
export type TraversalOutcome={complete:boolean;emptyConfirmed:boolean;receivedItems:number};

export const currentPrioritySequence:readonly AcquisitionWorkClass[]=['finalization','current_global','finalization','current_global'];
...
export function nextBootstrapWork(state:{bootstrap_state:string;recent_catchup_state:string;deep_history_state:string;current_stage:string},counter=0,phase:'current'|'historical'='current'):AcquisitionWorkClass|null{
  if(phase==='current')return currentPrioritySequence[counter%currentPrioritySequence.length];
...
export function recentComparableDurations<T extends {started:Date;ended:Date}>(rows:readonly T[],limit=5){return rows.filter(row=>row.ended>=row.started).sort((left,right)=>right.started.getTime()-left.started.getTime()).slice(0,limit).map(row=>row.ended.getTime()-row.started.getTime());}
...
export class DurableAcquisitionOrchestrator{
  ...
  async chooseWork(providerChampionshipId:string,dispatchCounter=0,phase:'current'|'historical'='current'){return (await this.resolveWorkUnit(providerChampionshipId,dispatchCounter,phase)).workClass;}

  private async resolveWorkUnit(providerChampionshipId:string,dispatchCounter=0,phase:'current'|'historical'='current'){const state=await this.ensureState(providerChampionshipId),candidate=nextBootstrapWork(state,dispatchCounter,phase),local=civilParts(this.clock.now(),state.provider_timezone);if(candidate!=='finalization')return {workClass:candidate,season:candidate==='deep_history'?Number(state.deep_history_season):local.year};const seasons=(await pool.query(`select distinct season from provider_source_entities where provider_championship_id=$1 and theoretical_end_at is not null and theoretical_end_at<=$2 and theoretical_end_at+make_interval(days=>$3)>=$2 and lower(coalesce(source_data->>'status',''))<>all($4::text[]) order by season`,[providerChampionshipId,this.clock.now(),Number(state.acquisition_finalization_grace_days),finalStatuses])).rows.map(row=>Number(row.season));if(!seasons.length)return {workClass:'current_global' as const,season:local.year};const cursor=state.finalization_cursor_season==null?null:Number(state.finalization_cursor_season),season=seasons.find(value=>cursor===null||value>cursor)??seasons[0];return {workClass:'finalization' as const,season};}
  ...
  async executeLease<P extends JsonObject,S extends JsonObject,C extends JsonObject>(input:{providerInstanceId:string;providerChampionshipId:string;season:number;dispatchCounter?:number;lease:{streamId:string;runId:string;workerId:string;generation:number};adapter:ProviderAdapter<P,S,C,AcquiredProviderSourceItem>;fetchInput:FetchWorkUnitInput<P,S,C>}){
    const state=await this.ensureState(input.providerChampionshipId);const phase=(await pool.query('select phase from sync_streams where id=$1',[input.lease.streamId])).rows[0]?.phase;if(phase!=='current'&&phase!=='historical')throw new Error('Stream acquisition absent.');const selection=await this.resolveWorkUnit(input.providerChampionshipId,input.dispatchCounter??0,phase),workClass=selection.workClass;if(!workClass)throw new Error('Aucun travail d’acquisition éligible.');const expectedSeason=selection.season;if(input.season!==expectedSeason||input.fetchInput.season!==expectedSeason)throw new Error(`Saison contradictoire : ${expectedSeason} attendue.`);if(input.fetchInput.phase!==phase)throw new Error(`Phase contradictoire : ${phase} attendue.`);
    const safeUnitKey=`${workClass}:${expectedSeason}`;const resumable=(await pool.query(`select id from provider_acquisition_traversals where stream_id=$1 and season=$2 and safe_unit_key=$3 and status in('running','partial') and complete=false order by started_at desc limit 1`,[input.lease.streamId,expectedSeason,safeUnitKey])).rows[0]?.id;const outcome=await this.transactions.executeUnit({...input,season:expectedSeason,workClass,safeUnitKey,traversalId:resumable,adapter:input.adapter,fetchInput:{...input.fetchInput,season:expectedSeason},afterPersist:async(client,result,totals)=>{if(workClass==='current_global')await this.classifyCurrentEntities(client,totals.traversalId,Number(state.acquisition_current_hot_days),state.provider_timezone);await this.recordOutcomeClient(client,input.providerChampionshipId,workClass,{complete:totals.complete,emptyConfirmed:totals.complete&&totals.receivedItems===0,receivedItems:totals.receivedItems},expectedSeason);}});return {...outcome,workClass,season:expectedSeason};
  }
  ...
  private async recordOutcomeClient(client:PoolClient,providerChampionshipId:string,workClass:AcquisitionWorkClass,outcome:TraversalOutcome,season?:number){const row=(await client.query(`select state.*,champ.acquisition_history_mode,champ.acquisition_history_from_season,champ.acquisition_empty_season_limit from provider_acquisition_state state join provider_championships champ on champ.id=state.provider_championship_id where state.provider_championship_id=$1 for update of state`,[providerChampionshipId])).rows[0];if(!row)throw new Error('État acquisition absent.');
      ...
      else if(workClass==='finalization'&&outcome.complete)await client.query(`update provider_acquisition_state set finalization_last_checked_at=$2,finalization_cursor_season=$3,updated_at=$2 where provider_championship_id=$1`,[providerChampionshipId,this.clock.now(),season]);
      return row;
  }

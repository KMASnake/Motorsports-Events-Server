...
import { deleteEvent, loadEventWorkspace, saveEvent, setEventPublication } from './eventApi';
...
import { emptyEventForm, eventToForm, filterEvents, formDateForDay, monthLabel, slugify } from './eventUtils';
...
const initialView = (): EventView => sessionStorage.getItem('mse-events-view') === 'list' ? 'list' : 'calendar';
...
export function EventsPage() {
  ...
  function startDuplicate(event: EventRow) {
    ...
    setEditing(null); setForm({ ...copy, name: `${copy.name} — copie`, slug: `${copy.slug}-copie` }); setFormError(null); setEditorOpen(true);
  }
  ...
  return <>
    ...
    {loading ? <div className="events-loading">Chargement des événements…</div> : <div className="events-workspace">
      <main>{view === 'calendar'
        ? <EventCalendarView month={month} events={filtered} championships={championships} selectedId={selectedId} onSelect={(event) => setSelectedId(event.id)} onCreateAt={startCreate} />
        : <EventListView events={filtered} championships={championships} selectedId={selectedId} onSelect={(event) => setSelectedId(event.id)} onEdit={startEdit} onDelete={(event) => void remove(event)} onTogglePublication={(event) => void togglePublication(event)} />}
      </main>
      <EventDetailsPanel event={selected} championships={championships} onEdit={startEdit} onDuplicate={startDuplicate} onDelete={(event) => void remove(event)} />
    </div>}

...
describe('normalization mapping reconstruction',()=>{
  ...
  it('accepts a data-backed future canonical session type without a code enum change',()=>expect(parseNormalizationMappingDocument({...document,sessionTypes:{Stage:'special_stage',Future:'super_special'}}).sessionTypes).toEqual({Stage:'special_stage',Future:'super_special'}));
  it('rejects session type targets that are not bounded canonical keys',()=>expect(()=>parseNormalizationMappingDocument({...document,sessionTypes:{Bad:'Special Stage'}})).toThrow(NormalizationMappingRepositoryError));

...
docker compose run --rm migrate >/dev/null
docker compose exec -T postgres psql -U mse -d motorsports_events -Atqc \
  "select version from schema_migrations where version='0009_provider_discovery'" \
  | grep -qx '0009_provider_discovery'
echo "Rollback puis réapplication 0009 : OK"

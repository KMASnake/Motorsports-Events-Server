...
PROJECT=${LOT44_UI_PROJECT:-mse-lot44-auth-ui}
POSTGRES_PORT=${LOT44_UI_POSTGRES_PORT:-55466}
API_PORT=${LOT44_UI_API_PORT:-3571}
WEB_PORT=${LOT44_UI_WEB_PORT:-3570}
PASSWORD=${LOT44_UI_POSTGRES_PASSWORD:-lot44-ui-test}
AUTH_SECRET=${LOT44_UI_AUTH_SECRET:-lot44-ui-hmac-secret-at-least-thirty-two-characters}
SESSION_SECRET=${LOT44_UI_SESSION_SECRET:-lot44-ui-session-secret-at-least-thirty-two-characters}
TEST_WORKDIR=$(mktemp -d "${TMPDIR:-/tmp}/mse-lot44-ui.XXXXXX")
...
echo "Création de la pile UI Authentification isolée..."
stack_cleanup
...
docker run --rm -v "$PWD:/source:ro" -v "$TEST_WORKDIR:/work" -w /work node:22-alpine \
  sh -lc 'tar -C /source --exclude=.git --exclude=node_modules --exclude=test-results -cf - . | tar -C /work -xf - && npm ci'
docker run --rm --network host -e API_URL -e WEB_URL -v "$TEST_WORKDIR:/work" -w /work \
  mcr.microsoft.com/playwright:v1.62.1-noble \
  npx playwright test tests/ui/auth.spec.ts --project=chromium
echo "Tests Chromium Authentification Lot 4.4 étape 3 : OK"

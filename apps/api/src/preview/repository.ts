...
export interface ResourceRow{resourceType:ResourceType;resourceId:string;revision:number;lifecycle:'active'|'removed';state:Record<string,unknown>|null;promotedAt:string;sortKey:string;championshipId?:string|null}
...
export interface ListInput{resourceType:ResourceType;limit:number;snapshotSequence:number;after?:{sortKey:string;resourceId:string};championshipId?:string;championshipSlug?:string;allowedChampionshipIds?:string[];from?:string;to?:string;status?:string;sessionType?:string}
export interface PreviewRepository{snapshotBoundary():Promise<number>;oldestSnapshotSequence():Promise<number>;oldestChangeSequence():Promise<number|null>;resolveChampionship(slug:string):Promise<string|null>;list(input:ListInput):Promise<ResourceRow[]>;detail(type:ResourceType,id:string):Promise<ResourceRow|null>;changes(after:number,limit:number,includeData:boolean,allowedChampionshipIds?:string[]):Promise<ChangeRow[]>}

function row(value:Record<string,unknown>):ResourceRow{return {resourceType:String(value.resource_type) as ResourceType,resourceId:String(value.resource_id),revision:Number(value.revision),lifecycle:String(value.lifecycle) as 'active'|'removed',state:value.canonical_state as Record<string,unknown>|null,promotedAt:new Date(value.promoted_at as string|Date).toISOString(),sortKey:String(value.sort_key??''),championshipId:value.championship_id==null?null:String(value.championship_id)};}
...
export class PostgresPreviewRepository implements PreviewRepository{
  ...
  async resolveChampionship(slug:string){const result=await this.db.query('select id::text from championships where slug=$1 and active=true limit 1',[slug]);return result.rows[0]?.id??null;}
  async list(input:ListInput){
    ...
    if(input.allowedChampionshipIds)add(input.allowedChampionshipIds,n=>`s.championship_id=any($${n}::text[])`);
  ...
  async changes(after:number,limit:number,includeData:boolean,allowedChampionshipIds?:string[]){const values:unknown[]=[after,limit+1],entitlement=allowedChampionshipIds?' and v.championship_id=any($3::text[])':'';if(allowedChampionshipIds)values.push(allowedChampionshipIds);const result=await this.db.query(`select c.*,v.lifecycle,v.canonical_state,v.championship_id,v.published_at as promoted_at,coalesce(v.canonical_state->>'startsAt',v.canonical_state->>'name','') sort_key from public_change_log c left join public_resource_versions v on v.publication_sequence=c.sequence where c.sequence>$1${entitlement} order by c.sequence limit $2`,values);return result.rows.map(value=>{const record=value as Record<string,unknown>;const current=includeData&&record.operation!=='removed'&&record.canonical_state?row({...record,revision:record.resource_revision}):null;return {sequence:Number(record.sequence),resourceType:String(record.resource_type) as ResourceType,resourceId:String(record.resource_id),revision:Number(record.resource_revision),operation:String(record.operation) as ChangeRow['operation'],changedFields:record.changed_fields as string[],occurredAt:new Date(record.occurred_at as string|Date).toISOString(),current};});}
}

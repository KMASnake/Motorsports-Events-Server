...
export class TheSportsDbAdapter extends DiscoveryAdapter{
  ...
  validateSourceConfig(value:unknown){const config=sourceConfig(value);if(!['league-season-v1','league-season-v2'].includes(config.strategy)||config.endpoint_template!==undefined||!/^\d{1,20}$/.test(config.external_id))throw new Error('Configuration TheSportsDB inconnue.');return {...config,strategy:'league-season-v1'};}

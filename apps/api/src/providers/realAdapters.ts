...
function sourceConfig(value:unknown):Source{const row=object(value);if(typeof row.strategy!=='string'||typeof row.external_id!=='string'||!row.external_id.trim())throw new Error('Source invalide.');if(row.endpoint_template!==undefined&&typeof row.endpoint_template!=='string')throw new Error('Endpoint invalide.');return {strategy:row.strategy,external_id:row.external_id,...(row.endpoint_template?{endpoint_template:row.endpoint_template}:{})};}
...
abstract class DiscoveryAdapter implements ProviderAdapter<Config,Source,Cursor,JsonObject>{
  ...
  championshipForm(){return [{key:'external_id',label:'Identifiant externe',type:'text' as const,required:true},{key:'strategy',label:'Stratégie',type:'text' as const,required:true},{key:'endpoint_template',label:'Modèle endpoint',type:'text' as const,required:false}];}
...
export class OcBlackTopAdapter extends DiscoveryAdapter{
  ...
  validateSourceConfig(value:unknown){const config=sourceConfig(value),entry=this.catalog.find(candidate=>candidate.externalId===config.external_id&&candidate.strategy===config.strategy&&candidate.endpointTemplate===config.endpoint_template);if(!entry)throw new Error('Stratégie OCBlackTop inconnue.');return config;}
...
export class TheSportsDbAdapter extends DiscoveryAdapter{
  ...
  validateSourceConfig(value:unknown){const config=sourceConfig(value);if(config.strategy!=='league-season-v1'||config.endpoint_template!==undefined)throw new Error('Stratégie TheSportsDB inconnue.');return config;}

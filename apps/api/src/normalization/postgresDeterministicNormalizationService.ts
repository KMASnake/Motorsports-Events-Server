...
export class PostgresDeterministicNormalizationService{
  ...
  async normalizeUnitInTransaction(client:PoolClient,input:NormalizeUnitInput){
    ...
    const candidateRows=envelope.kind==='event'?(await client.query(`select event.id,event.championship_id,extract(year from event.starts_at)::int season,relation.meeting_id,
      ...
      order by event.starts_at,event.id limit 51`,[input.mapping.championshipIds[envelope.championshipSourceId]??'',envelope.season])).rows
    ...
    let result=normalize(envelope,input.mapping,candidates,existing?String(existing):null,rejected);

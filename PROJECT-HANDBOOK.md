...
Le Concept, le contrat UI et l'Acceptance du **Lot 5.6 — Acquisition fournisseur durable** ont été formalisés, audités contre 5.4/5.5, la baseline sécurité et la frontière 5.7, puis consolidés après fermeture des constats. Le mainteneur a **explicitement validé et autorisé l'implémentation du Lot 5.6 le 2026-08-14**.

`authorized_sub_lot = 5.6` désigne le sous-lot actuellement autorisé à entrer en implémentation. Cette autorisation ne vaut ni validation de l'implémentation, ni autorisation de fusion dans `main`. Les **Lots 5.7 et suivants restent non autorisés** ; seules leur discussion, conception et documentation peuvent être préparées avant un nouveau gate explicite.

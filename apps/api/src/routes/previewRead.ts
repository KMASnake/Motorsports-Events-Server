...
export async function previewReadRoutes(app:FastifyInstance,options:PreviewReadOptions){
  ...
  const list=async(type:ResourceType,request:FastifyRequest,reply:FastifyReply)=>{
    ...
    const rows=await repository.list({resourceType:type,limit:query.limit,snapshotSequence,after:cursor?{sortKey:cursor.sortKey,resourceId:cursor.resourceId}:undefined,championshipId:query.championship_id,championshipSlug:query.championship,from,to:query.to,status:query.status,sessionType:query.session_type});
    const hasMore=rows.length>query.limit,visible=rows.slice(0,query.limit),last=visible.at(-1),issuedAt=Math.floor(now().valueOf()/1000);
    const nextCursor=hasMore&&last?encodeCursor({kind:'page',resourceType:type,snapshotSequence,sortKey:last.sortKey,resourceId:last.resourceId,filterHash:fingerprint,...(from?{effectiveFrom:from}:{}),issuedAt},options.cursorSecret):null;
    const syncCursor=encodeCursor({kind:'sync',sequence:snapshotSequence,issuedAt},options.cursorSecret);

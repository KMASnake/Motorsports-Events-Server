...
export class BoundedProviderOneShotRunner{
  ...
  async preflight(target:OneShotTarget){
    ...
    if(!row)return {status:'configuration_invalid' as const,reason:'exact_target_not_found',configuration_ready:false,execution_ready:false,configuration_blockers:['exact_target_not_found'],execution_blockers:['configuration_not_ready'],provider_requests_budget:target.maxProviderRequests,provider_requests_emitted:0,provider_requests_remaining:target.maxProviderRequests,PROVIDER_CALLS:0};
    const adapter=this.providers.registry.get(row.adapter_key);let runtimeMappingIdentity:string|null=null,providerConfigValid=false,sourceValid=false,resumableTraversalId:string|null=null,effectiveMappingUuid:string|null=null,mappingSource:'active'|'traversal_binding'='active',configurationReason:string|undefined;
    if(adapter){try{adapter.validateProviderConfig(row.provider_config);providerConfigValid=true;}catch{/* reported as a configuration blocker */}try{adapter.validateSourceConfig(row.source_config,{providerConfig:row.provider_config});sourceValid=true;}catch{/* reported as a configuration blocker */}}
    try{const context=await this.orchestrator.resolveTraversalContext(target.providerChampionshipId,target.streamId,new Date().getUTCFullYear(),currentPrioritySequence.indexOf('current_global'));resumableTraversalId=context.resumableTraversalId;effectiveMappingUuid=context.boundMappingVersionId??row.mapping_version_id;mappingSource=context.resumableTraversalId?'traversal_binding':'active';if(effectiveMappingUuid)runtimeMappingIdentity=(await this.mappings.resolveMappingConfig(effectiveMappingUuid)).version;}catch(error){configurationReason=String((error as {code?:string}).code??'mapping_resolution_failed');}
    const configurationBlockers:string[]=[];if(!adapter)configurationBlockers.push('adapter_unavailable');if(!providerConfigValid)configurationBlockers.push('provider_config_invalid');if(!sourceValid)configurationBlockers.push('source_config_invalid');if(row.phase!=='current')configurationBlockers.push('current_stream_absent');if(configurationReason)configurationBlockers.push(configurationReason);else if(!effectiveMappingUuid)configurationBlockers.push('mapping_absent');if(!row.credential_present)configurationBlockers.push('credential_missing');if(!row.quota_configuration)configurationBlockers.push('quota_configuration_missing');
    const configurationReady=configurationBlockers.length===0,executionBlockers:string[]=[];if(!configurationReady)executionBlockers.push('configuration_not_ready');if(!row.provider_enabled)executionBlockers.push('provider_disabled');if(row.provider_state!=='active')executionBlockers.push(row.provider_state==='paused'?'provider_paused':`provider_${row.provider_state}`);if(row.sync_state!=='active')executionBlockers.push('championship_inactive');if(!['ready','pending','running'].includes(row.stream_state))executionBlockers.push(`stream_${row.stream_state}`);const executionReady=configurationReady&&executionBlockers.length===0;

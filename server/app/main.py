...
app = FastAPI(
    ...
    version="2.4.0",
    description=f"Serveur central de {settings.project_name}.",
)
...
@app.get("/api/v1/sports", dependencies=[Depends(require_public_key)])
def sports(db: OrmSession = Depends(get_db)):
    return db.query(Sport).filter_by(enabled=True).order_by(Sport.name).all()


@app.get("/api/v1/events", dependencies=[Depends(require_public_key)])
def events(
    ...
    if not include_deleted:
        query = query.filter(Session.deleted.is_(False))

    if from_date:
        query = query.filter(Session.end_at >= from_date)

    if to_date:
        query = query.filter(Session.start_at <= to_date)

    if season is not None:
        query = query.filter(func.extract("year", Session.start_at) == season)
    ...
    if is_race is not None:
        query = query.filter(Session.is_race.is_(is_race))
    ...
    rows = query.order_by(Session.start_at).distinct().all()
...
@app.get("/api/v1/events/changes", dependencies=[Depends(require_public_key)])
def changes(since: datetime, db: OrmSession = Depends(get_db)):
    rows = db.query(Session).filter(
        Session.updated_at > since
    ).order_by(Session.updated_at).all()
    return {
        "updated": [
            {
                "id": row.id,

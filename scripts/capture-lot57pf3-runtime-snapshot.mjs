...
if(args.length!==20)fail('usage: capture-lot57pf3-runtime-snapshot.mjs --n-api-image REF@sha256:DIGEST --n-api-oci-layout DIR --n-web-image REF@sha256:DIGEST --n-web-oci-layout DIR --n-plus-one-api-image REF@sha256:DIGEST --n-plus-one-api-oci-layout DIR --n-plus-one-web-image REF@sha256:DIGEST --n-plus-one-web-oci-layout DIR --runtime-release n|n-plus-one --output FILE');
...
const layouts={n:{api:value('--n-api-oci-layout'),web:value('--n-web-oci-layout')},n_plus_1:{api:value('--n-plus-one-api-oci-layout'),web:value('--n-plus-one-web-oci-layout')}};
...
if(!runtimeRelease||!output||Object.values(refs).some(release=>Object.values(release).some(ref=>!ref))||Object.values(layouts).some(release=>Object.values(release).some(layout=>!layout)))fail('all arguments are mandatory');
...
const dockerMaterialization=(row,ref,chain,label)=>{if(!Array.isArray(row.RepoDigests)||!row.RepoDigests.includes(ref))fail(`${label} immutable locator is not present in RepoDigests`);const descriptor=row.Descriptor;if(descriptor!==undefined){if(descriptor?.digest!==chain.oci_locator.locator_index_digest||descriptor?.mediaType!==chain.locator_media_type)fail(`${label} Docker descriptor differs from the verified OCI locator`);if(row.Id!==descriptor.digest)fail(`${label} Docker Id differs from its verified descriptor`);}else if(row.Id!==chain.config_digest)fail(`${label} Docker Id is not a provable classic config identity`);return {docker_image_id:row.Id,descriptor_digest:descriptor?.digest??null,descriptor_media_type:descriptor?.mediaType??null};};
...
const inspectImage=(ref,layout,platform)=>{const locatorDigest=immutable.exec(ref)?.[1];const readRaw=createLocalOciReader({layout,locatorDigest:`sha256:${locatorDigest}`,fail});const chain=resolveOciRuntimeIdentity({ref,platform,readRaw,fail});const rows=json(['image','inspect',ref],`local image ${ref}`);if(!Array.isArray(rows)||rows.length!==1)fail(`local image ${ref} is ambiguous`);const row=rows[0];if(row.Os!==platform.split('/')[0]||row.Architecture!==platform.split('/')[1])fail(`local image ${ref} platform differs from ${platform}`);const materialization=dockerMaterialization(row,ref,chain,`local image ${ref}`);if(row.RootFS?.Type!=='layers'||JSON.stringify(row.RootFS.Layers)!==JSON.stringify(chain.rootfs_diff_ids))fail(`local image ${ref} rootfs differs from its verified OCI config`);const metadata=envMap(row);for(const key of ['APP_VERSION','GIT_SHA','BUILD_TIME'])if(!metadata[key]||metadata[key].toLowerCase()==='unknown')fail(`local image ${ref} ${key} is unknown`);if(!/^[0-9a-f]{40}$/.test(metadata.GIT_SHA)||Number.isNaN(Date.parse(metadata.BUILD_TIME)))fail(`local image ${ref} metadata is invalid`);return {...chain,docker_materialization:materialization,version:metadata.APP_VERSION,git_sha:metadata.GIT_SHA,git_tree:gitTree(metadata.GIT_SHA),build_time:metadata.BUILD_TIME};};
const platformOf=(container,label)=>{if(typeof container.Platform!=='string'||!container.Platform)fail(`${label} runtime platform is unknown`);const rows=json(['image','inspect',container.Image],`${label} runtime image`);if(!Array.isArray(rows)||rows.length!==1)fail(`${label} runtime image is ambiguous`);const image=rows[0];if(typeof image.Os!=='string'||!image.Os||typeof image.Architecture!=='string'||!image.Architecture)fail(`${label} runtime image platform is unknown`);if(container.Platform!==image.Os)fail(`${label} container OS differs from runtime image OS`);return `${image.Os}/${image.Architecture}${image.Variant?`/${image.Variant}`:''}`;};
const releases={n:{api:inspectImage(refs.n.api,layouts.n.api,platformOf(api,'API')),web:inspectImage(refs.n.web,layouts.n.web,platformOf(web,'Web'))},n_plus_1:{api:inspectImage(refs.n_plus_1.api,layouts.n_plus_1.api,platformOf(api,'API')),web:inspectImage(refs.n_plus_1.web,layouts.n_plus_1.web,platformOf(web,'Web'))}};
...
if(certification.Image!==expectedRuntime.api.docker_materialization.docker_image_id)fail(`certification runner does not use the selected ${runtimeRelease} API Docker image`);
if(api.Image!==expectedRuntime.api.docker_materialization.docker_image_id)fail(`runtime API does not use the selected ${runtimeRelease} API Docker image`);
if(web.Image!==expectedRuntime.web.docker_materialization.docker_image_id)fail(`runtime Web does not use the selected ${runtimeRelease} Web Docker image`);
if(worker.Image!==expectedRuntime.api.docker_materialization.docker_image_id)fail(`stopped worker does not identify the selected ${runtimeRelease} API Docker image`);

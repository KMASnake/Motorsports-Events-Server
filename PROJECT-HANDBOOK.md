...
`authorized_sub_lot = 5.7-P` autorise la tranche verticale de normalisation Production Preview depuis le 2026-08-21. Sa conception technique, son Acceptance PP-T01 à PP-T42 et ses six gates A→F sont validés par le mainteneur. Seul `authorized_technical_sub_lot = 5.7-P-A` est ouvert et reste non commencé ; il couvre exclusivement les fondations persistantes sans normalisation ni exposition client. La validation 5.6 et l’autorisation 5.7-P-A ne valent pas autorisation de fusion dans `main`. Le **Lot 5.7 complet et les Lots 5.8+ restent non autorisés**.

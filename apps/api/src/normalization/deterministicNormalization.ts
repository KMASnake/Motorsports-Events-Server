...
export interface IdentityContext {parentSourceRequired:boolean;parentCanonicalResolved:boolean;championshipSeasonId?:string|null;venueId?:string|null;venueLayoutId?:string|null}
...
export function resolveIdentity(state:NormalizedState,candidates:readonly MatchCandidate[],existingLink:string|null,rejectedTargetIds:readonly string[]=[]):Resolution{
  ...
  if(state.championshipId===null||state.championshipSeasonId===null||(state.circuitId===null&&state.venueId===null))return {decision:'review',targetId:null,score:null,signals:[],reason:state.championshipSeasonId===null?'championship_season_unresolved':'required_identity_unknown'};
  ...
  if(state.resourceKind==='meeting'){
    const plausible=candidates.filter(item=>item.championshipId===state.championshipId&&item.championshipSeasonId===state.championshipSeasonId&&(!state.round||!item.round||item.round===state.round)).sort((a,b)=>a.id.localeCompare(b.id,'en'));
    ...
    if(identityMatches.length===1)return {decision:'linked',targetId:identityMatches[0].id,score:null,signals:['championship','championship_season',...(state.round?['round']:['name','starts_at'])],reason:'deterministic_meeting_identity'};
    if(identityMatches.length>1)return {decision:'review',targetId:null,score:null,signals:['championship','championship_season'],reason:'ambiguous_meeting'};
...
export function normalize(input:SourceEnvelope,config:MappingConfig,candidates:readonly MatchCandidate[],existingLink:string|null,rejectedTargetIds:readonly string[]=[],identityContext?:IdentityContext){const state={...mapSource(input,config),championshipSeasonId:identityContext?.championshipSeasonId??null,venueId:identityContext?.venueId??null,venueLayoutId:identityContext?.venueLayoutId??null},resolution=identityContext?.parentSourceRequired&&!identityContext.parentCanonicalResolved&&!existingLink?{decision:'review',targetId:null,score:null,signals:['parent_source'],reason:'parent_identity_unresolved'} as const:resolveIdentity(state,candidates,existingLink,rejectedTargetIds);return {state,resolution,candidateId:stableUuid('mse-normalized-candidate',{sourceId:input.id,sourceHash:input.sourceHash,version:config.version}),proposedUuid:resolution.decision==='create'?stableUuid('mse-normalized-identity',{sourceId:input.id,kind:input.kind}):resolution.targetId,checksum:stableHash({state,resolution})};}

...
export async function previewReadRoutes(app:FastifyInstance,options:PreviewReadOptions){
  ...
  for(const type of ['championship','event','meeting'] as const){const plural=`${type}s`;app.get(`/api/v1/${plural}`,async(request,reply)=>list(type,request,reply));app.get(`/api/v1/${plural}/:id`,async(request,reply)=>{const parsed=uuid.safeParse((request.params as {id?:unknown}).id);if(!parsed.success)return error(reply,request,400,'invalid_request','Invalid resource identifier.');const found=await repository.detail(type,parsed.data),principal=options.principal?.(request),championshipId=found?.championshipId??String(found?.state?.championshipId??'');if(!found||(principal&&!principal.championshipIds.has(championshipId)))return error(reply,request,404,'not_found','Resource not found.');return publicState(found);});}

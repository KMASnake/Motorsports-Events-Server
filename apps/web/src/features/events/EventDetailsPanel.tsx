...
export function EventDetailsPanel({ event, championships, onEdit, onDuplicate, onDelete, onResize }: Props) {
  ...
  return <aside className="event-details" style={{ '--event-color': eventColor(event, championships) } as React.CSSProperties}>
    ...
    <div className="event-details-identity"><img src={assetRegistry.championship(event.championship_slug,event.championship_logo_url).src} alt={assetRegistry.championship(event.championship_slug,event.championship_logo_url).alt}/><div><small>{event.championship_name}</small><h3>{event.meeting_name ?? event.name}</h3></div></div>
    <dl>
      <div><dt>Championnat</dt><dd>{event.championship_name}</dd></div>
      <div><dt>Épreuve</dt><dd>{event.meeting_name ?? event.name}</dd></div>
      ...
      <div><dt>Intitulé de session</dt><dd>{event.session_title ?? (event.meeting_name ? event.name : 'Non défini')}</dd></div>

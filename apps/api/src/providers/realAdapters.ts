...
export class TheSportsDbAdapter extends DiscoveryAdapter{
  readonly key='thesportsdb';readonly expectedHost='www.thesportsdb.com';readonly defaultBaseUrl='https://www.thesportsdb.com/api/v2/json';
  ...
  validateProviderConfig(value:unknown){const config=baseConfig(value,this.expectedHost,true);return {...config,base_url:'https://www.thesportsdb.com/api/v2/json'};}
  championshipForm():readonly ProviderFieldSchema[]{return [{key:'external_id',label:'Identifiant de ligue',type:'text',required:true,help:'Identifiant numérique TheSportsDB.'},{key:'strategy',label:'Stratégie',type:'select',required:true,options:[{value:'league-season-v2',label:'Calendrier complet de ligue v2'}]}];}
  validateSourceConfig(value:unknown){const config=sourceConfig(value);if(!['league-season-v1','league-season-v2'].includes(config.strategy)||config.endpoint_template!==undefined||!/^\d{1,20}$/.test(config.external_id))throw new Error('Configuration TheSportsDB inconnue.');return {...config,strategy:'league-season-v2'};}
  protected headers(context:ProviderAdapterContext<Config>){const key=context.credentials.api_key;if(!key)throw new Error('Clé API TheSportsDB absente.');return {accept:'application/json','x-api-key':key};}
  private secureV2Config(context:ProviderAdapterContext<Config>):Config{return {...context.providerConfig,base_url:'https://www.thesportsdb.com/api/v2/json'};}
  private v2Context(context:ProviderAdapterContext<Config>):ProviderAdapterContext<Config>{return {...context,providerConfig:this.secureV2Config(context)};}
  async testConnection(context:ProviderAdapterContext<Config>){await this.json('/all/leagues',this.v2Context(context));return {ok:true,message:'Connexion TheSportsDB réussie.',checkedAt:new Date().toISOString()};}
  async discoverChampionships(context:ProviderAdapterContext<Config>):Promise<ChampionshipDiscoveryResult<Source>>{const payload=object(await this.json('/all/leagues',this.v2Context(context))),rows=Array.isArray(payload.leagues)?payload.leagues:[],items=[];for(const raw of rows){const row=object(raw),sport=String(row.strSport??'').toLowerCase();if(!sport.includes('motor'))continue;const id=String(row.idLeague??'').trim(),name=String(row.strLeague??id).trim();if(id&&name)items.push({externalChampionshipId:id,name,sourceConfig:{strategy:'league-season-v2',external_id:id},metadata:{sport:String(row.strSport??''),provenance:'provider-discovered'}});}return {items,complete:context.providerConfig.discovery_complete===true,provenance:'provider-discovered'};}
  async fetchWorkUnit(input:FetchWorkUnitInput<Config,Source,Cursor>):Promise<FetchWorkUnitResult<AcquiredProviderSourceItem,Cursor>>{const source=this.validateSourceConfig(input.sourceConfig);const payload=await this.json(`/schedule/league/${encodeURIComponent(source.external_id)}/${encodeURIComponent(String(input.season))}`,this.v2Context(input),{},input.signal);const rows=providerRows(payload,['events','schedule']);const parsed=validateProviderSourceItems({rows,season:input.season,entityKind:'event',idKeys:['idEvent','id','event_id']});return {status:'complete',items:parsed.items,itemAnomalies:parsed.anomalies,nextCursor:input.cursor,requestCount:1,complete:true,completionReason:rows.length===0?'explicit_empty_scope':'end_of_collection'};}
}

...
def snapshot(**overrides):
    value = {
        ...
        "releases": {
            "n": {"version": "1.0.0", "git_sha": "1" * 40, "build_time": "2026-08-28T10:00:00Z", "image_id": "sha256:" + "3" * 64, "image_digest": "sha256:" + "1" * 64},
            "n_plus_1": {"version": "1.0.1", "git_sha": "2" * 40, "build_time": "2026-08-28T11:00:00Z", "image_id": "sha256:" + "4" * 64, "image_digest": "sha256:" + "2" * 64},
        },
    }
...
if a[0]=='compose' and 'ps' in a:
    service=a[-1];print({'postgres':'postgres-id','api':'api-id','worker':'worker-id'}[service]);sys.exit(0)
...
if a[:2]==['image','inspect']:
    ref=a[2];digit='1' if 'release-n@' in ref else '2'
    print(json.dumps([{'Id':'sha256:'+digit*64,'RepoDigests':[ref],'Config':{'Env':['APP_VERSION=1.0.'+digit,'GIT_SHA='+digit*40,'BUILD_TIME=2026-08-28T1'+digit+':00:00Z']}}]));sys.exit(0)
sys.exit(3)
""", encoding="utf-8")
...
def run_runtime_probe(tmp_path, mode="safe", extra_args=()):
    ...
    n = "registry.example/release-n@sha256:" + "1" * 64
    n1 = "registry.example/release-n1@sha256:" + "2" * 64
    command = ["node", str(RUNTIME_PROBE), "--n-image", n, "--n-plus-one-image", n1, "--output", str(output), *extra_args]
...
class Lot57Pf3ToolingTests(unittest.TestCase):
    ...
    def test_preflight_rejects_unknown_or_mutable_release_identity(self):
        for release, field, value in [("n", "git_sha", "unknown"), ("n_plus_1", "build_time", "unknown"), ("n", "image_digest", "latest")]:
            with self.subTest(release=release, field=field):
                data = snapshot()
                data["releases"][release][field] = value
    ...
    def test_evidence_builder_independently_refuses_invalid_final_invariants(self):
        mutations = (
            ...
            lambda value: value["release"]["n"].update(git_sha="ABC"),
            lambda value: value["release"]["n"].update(image_digest="latest"),
            lambda value: value["release"]["n_plus_1"].update(image_digest=value["release"]["n"]["image_digest"]),
            lambda value: value["release"]["n"].update(build_time="unknown"),
        )
        for index, mutate in enumerate(mutations):
            with self.subTest(case=index), tempfile.TemporaryDirectory() as directory:
                raw, source, output = evidence_input(), Path(directory) / "raw.json", Path(directory) / "evidence.json"

...
export type AcquisitionWorkClass='finalization'|'current_hot'|'current_future'|'recent_catchup'|'deep_history';
export type TraversalOutcome={complete:boolean;emptyConfirmed:boolean;receivedItems:number};

export const currentPrioritySequence:readonly AcquisitionWorkClass[]=['finalization','current_hot','finalization','current_hot','current_future'];
...
export function nextBootstrapWork(state:{bootstrap_state:string;recent_catchup_state:string;deep_history_state:string;current_stage:string},counter=0,phase:'current'|'historical'='current'):AcquisitionWorkClass|null{
  if(phase==='current')return state.current_stage==='future'?'current_future':currentPrioritySequence[counter%currentPrioritySequence.length];
...
export class DurableAcquisitionOrchestrator{
  ...
  async ensureState(providerChampionshipId:string){
    ...
    return (await pool.query(`select state.*,champ.acquisition_history_mode,champ.acquisition_history_from_season,champ.acquisition_empty_season_limit,champ.acquisition_current_hot_days,champ.acquisition_finalization_grace_days from provider_acquisition_state state join provider_championships champ on champ.id=state.provider_championship_id where state.provider_championship_id=$1`,[providerChampionshipId])).rows[0];
  }

  async chooseWork(providerChampionshipId:string,dispatchCounter=0,phase:'current'|'historical'='current'){const state=await this.ensureState(providerChampionshipId);return nextBootstrapWork(state,dispatchCounter,phase);}

  async executeLease<P extends JsonObject,S extends JsonObject,C extends JsonObject>(input:{providerInstanceId:string;providerChampionshipId:string;season:number;dispatchCounter?:number;lease:{streamId:string;runId:string;workerId:string;generation:number};adapter:ProviderAdapter<P,S,C,AcquiredProviderSourceItem>;fetchInput:FetchWorkUnitInput<P,S,C>}){await this.ensureState(input.providerChampionshipId);const phase=(await pool.query('select phase from sync_streams where id=$1',[input.lease.streamId])).rows[0]?.phase;if(phase!=='current'&&phase!=='historical')throw new Error('Stream acquisition absent.');const workClass=await this.chooseWork(input.providerChampionshipId,input.dispatchCounter??0,phase);if(!workClass)throw new Error('Aucun travail d’acquisition éligible.');const safeUnitKey=`${workClass}:${input.season}`;const resumable=(await pool.query(`select id from provider_acquisition_traversals where stream_id=$1 and season=$2 and safe_unit_key=$3 and status in('running','partial') and complete=false order by started_at desc limit 1`,[input.lease.streamId,input.season,safeUnitKey])).rows[0]?.id;const outcome=await this.transactions.executeUnit({...input,workClass,safeUnitKey,traversalId:resumable,adapter:input.adapter,fetchInput:input.fetchInput,afterPersist:(client,result)=>this.recordOutcomeClient(client,input.providerChampionshipId,workClass,{complete:result.complete,emptyConfirmed:result.completionReason==='explicit_empty_scope',receivedItems:result.items.length})});return {...outcome,workClass};}

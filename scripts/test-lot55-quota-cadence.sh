...
docker compose exec -T postgres psql -U mse -d motorsports_events -v ON_ERROR_STOP=1 -c "select 'Migrations 0013/0014/0015 et surfaces quota : OK' where exists(select 1 from schema_migrations where version='0015_lot55_final_audit_fixes') and exists(select 1 from information_schema.columns where table_name='provider_quota_runtime' and column_name='next_eligible_at') and exists(select 1 from information_schema.columns where table_name='provider_request_charges' and column_name='charge_sequence');"

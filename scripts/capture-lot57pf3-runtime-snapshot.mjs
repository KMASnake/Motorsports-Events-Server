...
const inspectImage=ref=>{const rows=json(['image','inspect',ref],`image ${ref}`);if(!Array.isArray(rows)||rows.length!==1)fail(`image ${ref} is ambiguous`);const row=rows[0],runtimeManifestDigest=ref.slice(ref.indexOf('@')+1);if(!Array.isArray(row.RepoDigests)||!row.RepoDigests.includes(ref))fail(`runtime manifest ${ref} is not locally verified`);if(typeof row.Id!=='string'||!/^sha256:[0-9a-f]{64}$/.test(row.Id))fail(`image ${ref} config digest is unknown`);if(row.RootFS?.Type!=='layers'||!Array.isArray(row.RootFS.Layers)||row.RootFS.Layers.length===0||row.RootFS.Layers.some(layer=>!/^sha256:[0-9a-f]{64}$/.test(layer)))fail(`image ${ref} rootfs identity is unknown`);const metadata=envMap(row);for(const key of ['APP_VERSION','GIT_SHA','BUILD_TIME'])if(!metadata[key]||metadata[key].toLowerCase()==='unknown')fail(`image ${ref} ${key} is unknown`);if(!/^[0-9a-f]{40}$/.test(metadata.GIT_SHA)||Number.isNaN(Date.parse(metadata.BUILD_TIME)))fail(`image ${ref} metadata is invalid`);return {runtime_ref:ref,runtime_manifest_digest:runtimeManifestDigest,config_digest:row.Id,rootfs_diff_ids:row.RootFS.Layers,version:metadata.APP_VERSION,git_sha:metadata.GIT_SHA,git_tree:gitTree(metadata.GIT_SHA),build_time:metadata.BUILD_TIME};};

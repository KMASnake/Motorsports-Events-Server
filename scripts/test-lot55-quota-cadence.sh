...
docker compose exec -T postgres psql -U mse -d motorsports_events -v ON_ERROR_STOP=1 -c "select 'Migrations 0013/0014 et surfaces quota : OK' where exists(select 1 from schema_migrations where version='0014_lot55_audit_fixes') and to_regclass('public.provider_request_charges') is not null;"

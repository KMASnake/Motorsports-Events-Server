...
docker compose exec -T postgres psql -U mse -d motorsports_events -v ON_ERROR_STOP=1 -c "select 'Rollback/réapplication 0011/0012 et données 5.3 conservées : OK' where to_regclass('public.provider_discovered_championships') is not null and to_regclass('public.sync_streams') is not null and exists(select 1 from information_schema.columns where table_name='provider_championships' and column_name='sync_state_before_championship_disable');"

...
export class AcquisitionTransactionService{
  ...
  private async persistUnit<C extends JsonObject>(client:PoolClient,input:UnitContext&{traversalId:string;result:FetchWorkUnitResult<AcquiredProviderSourceItem,C>}){
    ...
    for(const item of input.result.items){
      ...
      if(!previous||previous.source_hash!==sourceHash)await client.query(`insert into provider_source_changes(source_entity_id,traversal_id,change_type,field_name,old_value,new_value,origin,changed_at) values($1,$2,$3,'source_hash',$4::jsonb,$5::jsonb,'provider',$6)`,[entity.id,input.traversalId,previous?'source_updated':'source_created',previous?JSON.stringify(previous.source_hash):null,JSON.stringify(sourceHash),now]);
    }

...
docker run --rm -v "$PWD:/source:ro" -w /work node:22-alpine sh -lc 'cp -a /source/. /work/ && npm ci >/dev/null && npm run typecheck --workspace @mse/api && npm test --workspace @mse/api -- schedulerPolicy.test.ts providerSchedulerRoutes.test.ts'
docker compose exec -T postgres psql -U mse -d motorsports_events -v ON_ERROR_STOP=1 -c "do \$\$ begin if(select count(*) from schema_migrations where version='0011_persistent_sync_scheduler')<>1 then raise exception 'migration 0011 absente';end if;if(select count(*) from sync_streams)<>0 or (select count(*) from sync_runs)<>0 then raise exception 'fixtures scheduler non nettoyées';end if;end \$\$;"
docker compose run --rm migrate sh /migrations/migrate.sh down 0011_persistent_sync_scheduler >/dev/null;docker compose run --rm migrate >/dev/null
docker compose exec -T postgres psql -U mse -d motorsports_events -v ON_ERROR_STOP=1 -c "select 'Rollback/réapplication 0011 et données 5.3 conservées : OK' where to_regclass('public.provider_discovered_championships') is not null and to_regclass('public.sync_streams') is not null;"

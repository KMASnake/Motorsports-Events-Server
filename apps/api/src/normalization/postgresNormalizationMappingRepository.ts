...
export class PostgresNormalizationMappingRepository{
  ...
  async resolveTraversalMappingConfig(traversalId:string,client:PoolClient|typeof pool=pool){const version=await this.getTraversalMapping(traversalId,client);if(!version)throw new NormalizationMappingRepositoryError('traversal_mapping_absent','Traversal has no mapping binding.');return mappingConfigFromVersion(version);}
}

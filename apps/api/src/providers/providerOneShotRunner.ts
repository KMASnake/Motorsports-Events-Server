...
export class BoundedProviderOneShotRunner{
  ...
  async preflight(target:OneShotTarget){
    ...
    const adapter=this.providers.registry.get(row.adapter_key);let runtimeMappingIdentity:string|null=null,sourceValid=false,resumableTraversalId:string|null=null,effectiveMappingUuid:string|null=null,mappingSource:'active'|'traversal_binding'='active',configurationReason:string|undefined;
    try{sourceValid=Boolean(adapter?.validateSourceConfig(row.source_config,{providerConfig:row.provider_config}));const context=await this.orchestrator.resolveTraversalContext(target.providerChampionshipId,target.streamId,new Date().getUTCFullYear(),currentPrioritySequence.indexOf('current_global'));resumableTraversalId=context.resumableTraversalId;effectiveMappingUuid=context.boundMappingVersionId??row.mapping_version_id;mappingSource=context.resumableTraversalId?'traversal_binding':'active';if(effectiveMappingUuid)runtimeMappingIdentity=(await this.mappings.resolveMappingConfig(effectiveMappingUuid)).version;}catch(error){configurationReason=String((error as {code?:string}).code??'mapping_resolution_failed');}
    const valid=Boolean(adapter)&&sourceValid&&row.provider_enabled&&row.provider_state==='active'&&row.sync_state==='active'&&row.phase==='current'&&['ready','pending','running'].includes(row.stream_state)&&Boolean(effectiveMappingUuid)&&row.credential_present&&!configurationReason;
    return {status:valid?'preflight_ok' as const:'configuration_invalid' as const,...(!valid&&configurationReason?{reason:configurationReason}:{}),provider_instance_id:row.provider_instance_id,adapter_key:row.adapter_key,provider_enabled:row.provider_enabled,provider_state:row.provider_state,provider_championship_id:row.provider_championship_id,canonical_championship_id:row.championship_id,external_championship_id:row.external_championship_id,source_strategy:row.source_strategy,stream_id:row.stream_id,stream_phase:row.phase,stream_state:row.stream_state,active_mapping_uuid:row.mapping_version_id,resumable_traversal_id:resumableTraversalId,effective_mapping_uuid:effectiveMappingUuid,mapping_source:mappingSource,runtime_mapping_identity:runtimeMappingIdentity,credential_present:row.credential_present,quota_configuration:row.quota_configuration,active_lease:{owned:row.lease_owner!==null,owner:row.lease_owner,expires_at:row.lease_expires_at?.toISOString()??null},requested_max_provider_requests:target.maxProviderRequests,publication_control:{preview_api_enabled:process.env.PREVIEW_API_ENABLED==='true'},provider_requests_budget:target.maxProviderRequests,provider_requests_emitted:0,provider_requests_remaining:target.maxProviderRequests,PROVIDER_CALLS:0};
  }
